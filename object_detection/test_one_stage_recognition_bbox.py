...
def get_file_names(folder_path):
    ...
    # 合并后缀为jpg或者tif的文件列表
    all_files = jpg_files + tif_files + png_files + bmp_files
...
def fairv2_test(args):
    ...
    if not distributed:
        ...
        outputs = multi_gpu_test(model, data_loader, args.tmpdir,
                                 args.gpu_collect)
    # import ipdb;ipdb.set_trace()
    outputs = filter.filter_bboxes(outputs,thr=0.3)
    ...
    print(len(bboxes))
    sorted_bboxes = post_processing(bboxes)
...
def post_processing(bboxes):
    ...
    for bbox in tqdm(bboxes):
        ...
        if name not in name_set:
            name_set.add(name)
            bboxes_post[name] = []
            bboxes_post[name].append(bbox[1:])
        else:
            confidence1 = float(bbox[2])
            confidence2 = float(bboxes_post[name][0][1])
            if confidence1 - confidence2 > 0:
                bboxes_post[name][0] = (bbox[1:])
    
    sorted_dict = {k: bboxes_post[k] for k in sorted(bboxes_post.keys())}
...
def main():
    ...
    sorted_bboxes = fairv2_test(args)
    save_path = "/data5/laiping/tianzhibei/pretrain_datasets/船舶/卫星1/predict.json"
