...
results_folder_path = '/data5/laiping/tianzhibei/test-recogntion-11-11/output_path'
...
def get_points(file_path, is_gt):
    """从 XML 文件中获取目标的相关信息"""
...
def evaluate_image(result_path, gt_path):
    """对比同一图像的预测结果和真实标签"""
    result_objects = get_points(result_path, False)
    gt_objects = get_points(gt_path, True)
        ...
        for gt_obj in gt_objects:
            ...
            if iou >= iou_thresh:
                if r_obj.class_name == gt_obj.class_name:
                    assert gt_obj.class_name in class_score_list.keys(), f'lost class: {gt_obj.class_name}'
                    class_score_list[gt_obj.class_name].tp += 1.0
                else:
                    # print(r_obj.class_name , gt_obj.class_name)
...
if __name__ == '__main__':
    ...
    for result_file_path in tqdm(file_list):
        ...
        evaluate_image(result_file_path, gt_file_path)
